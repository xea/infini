...
public:
	...
	ActorSelection(std::shared_ptr<ActorRef> anchor, const std::deque<std::string> path) : anchor(anchor), path(path) {};
    // Send a message to the selected actor or actors.
...
ActorSelection::ActorSelection(std::shared_ptr<ActorRef> anchor, std::string path) {
	...
	// This is probably a very suboptimal way of splitting strings
	if (path.size() > 0) {
